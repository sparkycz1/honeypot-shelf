...
async def update_smtp_settings(
    ...
    """Saves the SMTP relay's connection details — see
    `app.services.smtp`/`app.services.notifications` for what actually
    sends through this (Notifications: per-user, per-honeypot honeypot-
    alert/unavailability email)."""
    app_settings = await get_or_create_app_settings(db)
    errors: list[str] = []
