...
def test_build_sudoers_grant_command_grants_raspi_config_and_honeypotshelf_scripts():
    command = build_sudoers_grant_command("pi")
    assert "/usr/bin/raspi-config" in command
    assert "/usr/bin/bash /tmp/.honeypotshelf-*" in command
...
def test_build_onboarding_command_includes_the_sudoers_grant():
    script = build_onboarding_command("ssh-ed25519 AAAAfake honeypotshelf")
